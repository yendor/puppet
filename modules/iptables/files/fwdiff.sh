...
/bin/sed -e 's/#.*//' -e 's/[ ^I]*$//' -e '/^$/ d' < /etc/firewall/puppet-firewall.conf | /usr/bin/perl -lne '($chain, $policy) = ($_ =~ /^:(\w+) (\w+) \[\d+:\d+\]$/); if ($chain) { print ":$chain $policy [0:0]"; } else { print $_; }' | /bin/grep -v 'fail2ban' > $GENFILE
